document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                submitBtn.style.background = 'var(--error-color, #ff4757)';
                
                // Show error notification with option to try alternative
                showNotification(`Failed to send message. <button onclick="sendEmailFallback('${templateParams.from_name}', '${templateParams.from_email}', '${templateParams.subject}', '${templateParams.message}')" style="color: var(--primary-color); background: none; border: none; cursor: pointer; text-decoration: underline;">Try alternative method</button>`, 'error');
    ...
    function showNotification(message, type = 'info') {
        ...
        // Auto-remove after 7 seconds (increased for error messages with buttons)
        setTimeout(() => {
    ...
    // Alternative: Fallback to mailto if EmailJS fails
    function sendEmailFallback(name, email, subject, message) {
        const mailtoLink = `mailto:${EMAIL_CONFIG.recipientEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(`From: ${name} (${email})\n\n${message}`)}`;
        window.location.href = mailtoLink;
        showNotification('Opening your default email client...', 'info');
    }

    // Make sendEmailFallback available globally
